...
class Results extends Component {
  constructor(props) {
   ...
   componentDidMount() {
     const { loanValue, propertyValue, propertyEstimatedRentalIncome, incomeTaxBand1 } = this.props.values;
   ...
   getMetaData(data = {}) {
    ...
    .then(res => res.json())
    .then(response => {
      for (var lenderMetaData of response.results ) {
        this.getBtlResults(lenderMetaData)
      }
    })
		.then( res => document.getElementById('loader').style.display = 'none')
    .catch(error => { document.getElementById('loader').style.display = 'none'; console.error('Error:', error)});// parses JSON response into native JavaScript objects
  }
  ...
  getBtlResults(data) {
   ...
   .then(response => {
     ...
     // this.setState({
     //      isLoaded: true,
     //      items: response.results.concat(items)
     //    });
